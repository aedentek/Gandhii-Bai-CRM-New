...
const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    ...
    // Create patient-specific directory: server/Photos/Doctor Admission/{patientId}
    const uploadPath = path.join(__dirname, '../Photos/Doctor Admission', patientId.toString());
...
router.post('/upload-patient-file', upload.single('file'), async (req, res) => {
  try {
    ...
    const relativePath = path.join('Photos', 'Doctor Admission', req.body.patientId || 'temp', req.file.filename);
...
router.post('/move-patient-files', async (req, res) => {
  ...
  try {
    ...
    const patientFolder = path.join(__dirname, '../Photos/Doctor Admission', patientId);
    ...
    for (const [fieldName, tempPath] of Object.entries(tempPaths)) {
      if (tempPath && typeof tempPath === 'string') {
        try {
          ...
          const newRelativePath = path.join('Photos', 'Doctor Admission', patientId, filename);
    ...
    try {
        const [rows] = await db.query(`
            SELECT 
                ...
                TIME_FORMAT(attendance.check_out_time, '%H:%i:%s') as check_out_time,
                attendance.notes,
...
// POST /api/patient-attendance
router.post('/patient-attendance', async (req, res) => {
    try {
        const { patientId, date, status, checkInTime, notes } = req.body;
        ...
        const [result] = await db.query(`
            ...
            VALUES (?, ?, ?, ?, ?)
        `, [
            patientId.replace('P', ''), // Remove 'P' prefix to get original ID
            date,
            status,
            checkInTime,
            notes || ''
...
router.post('/patient-attendance/check-in', async (req, res) => {
    try {
        ...
        if (existingRecord.length > 0) {
            ...
            await db.query(
                `UPDATE patient_attendance 
                 SET status = ?, check_in_time = ?, modified_time = NOW()
...
router.put('/patient-attendance/:id/status', async (req, res) => {
    try {
        ...
        await db.query(
            `UPDATE patient_attendance 
             SET status = ?, notes = ?, modified_time = NOW()
             WHERE id = ?`,
            [status, notes, id]
        );
...
const initializeDataFiles = async () => {
  try {
    await fs.access(PATIENT_CALL_RECORDS_PATH);
  } catch {
    await fs.writeFile(PATIENT_CALL_RECORDS_PATH, JSON.stringify([]));
  }
};
...
router.get('/patient-call-records', async (req, res) => {
  try {
    const data = await fs.readFile(PATIENT_CALL_RECORDS_PATH, 'utf8');
...
router.post('/patient-call-records', async (req, res) => {
  try {
    const records = JSON.parse(await fs.readFile(PATIENT_CALL_RECORDS_PATH, 'utf8'));
    const newRecord = {
      id: uuidv4(),
      ...req.body,
    ...
    await fs.writeFile(PATIENT_CALL_RECORDS_PATH, JSON.stringify(records, null, 2));
...
router.delete('/patient-call-records/:id', async (req, res) => {
  try {
    ...
    await fs.writeFile(PATIENT_CALL_RECORDS_PATH, JSON.stringify(filtered, null, 2));
    res.status(204).send();
  } catch (error) {
...
router.post('/patients', async (req, res) => {
  ...
  try {
    ...
    const [result] = await db.query(
      ...
      [
        patientId, name, age, gender, phone, email, address, emergencyContact, medicalHistory,
        admissionDate, status, attenderName, attenderPhone, attenderRelationship,
        photo, patientAadhar, patientPan, attenderAadhar, attenderPan, fees, bloodTest, pickupCharge, totalAmount, payAmount, balance,
        paymentType, fatherName, motherName, dateOfBirth, marriageStatus, employeeStatus
      ]
    ...
    console.log('Patient added successfully:', { id: result.insertId, patientId, name });
    res.status(201).json({ 
      id: result.insertId,
      patient_id: patientId,
      message: 'Patient added successfully',
      patient: { id: result.insertId, patient_id: patientId, name, ...req.body }
    });
  } catch (err) {
...
router.put('/patients/:id', async (req, res) => {
  ...
  try {
    ...
    console.log('Patient updated successfully:', { id: req.params.id, name });
    res.json({ 
      id: req.params.id, 
...
router.get('/patient-history', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM patient_history ORDER BY date DESC, created_at DESC');
    res.json(rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/patient-history/patient/:patientId', async (req, res) => {
  try {
    const [rows] = await db.query(
      'SELECT * FROM patient_history WHERE patient_id = ? ORDER BY date DESC, created_at DESC',
...
router.get('/patient-history/:id', async (req, res) => {
  try {
    const [rows] = await db.query('SELECT * FROM patient_history WHERE id = ?', [req.params.id]);
    if (rows.length === 0) {
...
// Medical History File Upload endpoint
router.post('/upload-medical-history-file', (req, res) => {
  console.log('📤 Medical history file upload request received');
  ...
  upload.single('file')(req, res, function (err) {
    if (err) {
      console.error('❌ Multer error:', err.message);
      return res.status(400).json({ 
        error: 'Medical history file upload error', 
        details: err.message 
      });
    }
    ...
    try {
      ...
      const medicalHistoryDir = path.join(__dirname, 'uploads', 'medical-history', patientId, subDir);
      ...
      // Move file to medical history directory
      const oldPath = req.file.path;
      const newFilename = `${Date.now()}-${req.file.originalname}`;
      const newPath = path.join(medicalHistoryDir, newFilename);
      
      fs.renameSync(oldPath, newPath);
      
      const filePath = `/uploads/medical-history/${patientId}/${subDir}/${newFilename}`;
      console.log('✅ Medical history file uploaded successfully:', filePath);
      ...
      console.error('❌ Error processing medical history upload:', error);
      res.status(500).json({ error: 'Medical history file upload processing failed' });
    }
  });
});
