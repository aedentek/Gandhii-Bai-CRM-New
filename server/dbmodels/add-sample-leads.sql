-- First, let's update the leads table to match the frontend status values
ALTER TABLE leads MODIFY COLUMN status ENUM('Closed', 'Reminder', 'Not Interested') DEFAULT 'Closed';

-- Insert sample lead categories first
INSERT INTO lead_categories (name, description, createdAt) VALUES
...
SELECT COUNT(*) as total_categories FROM lead_categories;

-- Show sample of inserted data
SELECT id, name, contactNumber, category, status, date FROM leads LIMIT 10;
